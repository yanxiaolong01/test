...
public class WxApiClient {
    ...
    // 获取jsTicket
    public static String getJSTicketCurr(MpAccount mpAccount, String type)throws WxErrorException {
        // 获取唯一的JSTicket，如果是多账号，请自行处理
        if(type.equals("jsapi")){
            ...
            if (jsTicket != null && !jsTicket.isExpires()) {// 不为空，并且没有过期
                ...
                String token = getAccessToken(mpAccount);
                jsTicket = WxApi.getJSTicketCurr(token, type);
                if (jsTicket != null) {
                    if (jsTicket.getErrcode() == null) {
                        WxMemoryCacheClient.addJSTicket(mpAccount.getAccount(), jsTicket);
                        return jsTicket.getTicket();
            ...
            WXTicket wxTicket = WxMemoryCacheClient.getWXTicket();
            if (wxTicket != null && !wxTicket.isExpires()) {// 不为空，并且没有过期
                return wxTicket.getTicket();
            } else {
                String token = getAccessToken(mpAccount);
                wxTicket = WxApi.getWXTicketCurr(token, type);
                if (wxTicket != null) {
                    if (wxTicket.getErrcode() == null) {
                        WxMemoryCacheClient.addWXTicket(mpAccount.getAccount(), wxTicket);
                        return wxTicket.getTicket();
                    }else{
                        throw new WxErrorException(WxError.newBuilder().setErrorCode(wxTicket.getErrcode()).setErrorMsg(wxTicket.getErrmsg()).build());
                    }
                }
                return null;
            }
    ...
    public static JSONObject addNewsMaterial(List<MsgNews> msgNewsList, String mediaId, MpAccount mpAccount)
        ...
        for (MsgNews news : msgNewsList) {
            ...
            jsonObj.put("need_open_comment", news.getOpencomment());
