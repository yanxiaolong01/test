...
public class WxApi {
    ...
    // js ticket CURR
    public static final String GET_JSAPI_TICKET_CURR = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=%s&type=%s";
    ...
    // 网页授权OAuth2.0获取code
    public static String getOAuthCodeUrl1(String appId, String redirectUrl, String code, String scope, String state) {
        return String.format(GET_OAUTH_CODE, appId, urlEnodeUTF8(redirectUrl), code, scope, state);
    }
    ...
    // 获取js ticket url curr
    public static String getJsApiTicketUrlCurr(String token,String type) {
        return String.format(GET_JSAPI_TICKET_CURR, token, type);
    }
    ...
    // 获取js ticket Curr
    public static JSTicket getJSTicketCurr(String token, String type) throws WxErrorException{
        JSTicket jsTicket = null;
        String jsTicketUrl = WxApi.getJsApiTicketUrlCurr(token, type);
        JSONObject jsonObject = httpsRequest(jsTicketUrl, HttpMethod.GET, null);
        if (null != jsonObject && jsonObject.containsKey("errcode") && jsonObject.getIntValue("errcode") == 0) {
            try {
                jsTicket = new JSTicket();
    ...
    // wx ticket Curr
    public static WXTicket getWXTicketCurr(String token, String type) throws WxErrorException{
        WXTicket wxTicket = null;
        String wxTicketUrl = WxApi.getJsApiTicketUrlCurr(token, type);
        JSONObject jsonObject = httpsRequest(wxTicketUrl, HttpMethod.GET, null);
        if (null != jsonObject && jsonObject.containsKey("errcode") && jsonObject.getIntValue("errcode") == 0) {
            try {
