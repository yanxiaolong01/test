...
public class WxMemoryCacheClient {
	...
	public static void addMpAccount(List<Account> accounts){
		...
		CacheUtils.put("session_account", "imoreway_service");
...
//
	 ...
	 * @return
	 */
	public static WXTicket addWXTicket(String account , WXTicket wxTicket){
		Map<String,WXTicket> accountWXTicketMap = (Map<String, WXTicket>) CacheUtils.get("accountWXTicketMap");
		if(null==accountWXTicketMap){
			accountWXTicketMap=Maps.newHashMap();
		}
		if(wxTicket != null){
			accountWXTicketMap.put(account, wxTicket);
		}
		CacheUtils.put("accountWXTicketMap", accountWXTicketMap);
		return wxTicket;
	}

	/**
	 * WXTicket的获取，绝对不要从缓存中直接获取，请从WXTicket中获取；
	 * @return
	 */
	public static WXTicket getWXTicket(){
		Map<String,WXTicket> accountWXTicketMap = (Map<String, WXTicket>) CacheUtils.get("accountWXTicketMap");
		if(null==accountWXTicketMap){
			accountWXTicketMap=Maps.newHashMap();
		}
		return accountWXTicketMap.get(getAccount());
	}
	...
	public static String getAccount() {
		String account = (String) CacheUtils.get(SESSION_ACCOUNT);
		return account;
	}
}
