...
public class MyServiceImpl implements MyService {
    ...
    public boolean syncAccountFansList(MpAccount mpAccount) throws WxErrorException {
        ...
        List<AccountFans> accountFans = fansDao.list(null);
        List<String> list = new ArrayList();
        for(AccountFans af : accountFans){
            list.add(af.getOpenId());
        }
        if (lastFans != null) {
            nextOpenId = lastFans.getOpenId();
        }
        return doSyncAccountFansList(nextOpenId, mpAccount,list);
    }

    // 同步粉丝列表(开发者在这里可以使用递归处理)
    private boolean doSyncAccountFansList(String nextOpenId, MpAccount mpAccount,List<String> list) throws WxErrorException {
        ...
        if (jsonObject.containsKey("data")) {
            if (jsonObject.getJSONObject("data").containsKey("openid")) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if(!list.contains((String)openId)){
